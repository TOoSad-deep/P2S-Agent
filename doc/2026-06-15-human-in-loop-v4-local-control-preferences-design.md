...
> **状态:** 技术方案，待评审。
>
> **依赖:** V1 branch refine、V2 branch workspace、V3 variant exploration、已完成的 [V2.1 Branch Canvas Workspace](2026-06-16-human-in-loop-v2-1-branch-canvas-workspace-design.md)。V4 的 controls/preferences 应优先挂入 Branch Canvas inspector，region/preference 由 V4 自己扩展为附属节点进入画布。V4 复用用户 feedback、branch lineage、variant ratings/winner events，并新增结构化约束和偏好记忆。
>
> **V4.5 补充:** [V4.5 局部叠加与融合优化](2026-06-16-human-in-loop-v4-5-local-fusion-design.md) 在 V4.2 region/mask 和 V3.5 batch draw 之后实施，用多张抽卡结果的局部优点生成 composite target，再通过 shader 定向优化输出统一 shader。
>
...
6. V4.5 起，用户可以从多张抽卡结果中选择局部优点，生成融合目标并优化为统一 shader。

## Non-goals

- 不训练私有模型，不做不可解释的在线学习。
- 不做像 Photoshop 一样的像素级直接编辑；仍然是 shader 生成/优化。V4.5 的局部叠加首版也不是最终 PNG 拼贴，而是 composite target + shader refine。
...
| V4.5 | 局部叠加与融合优化 | 从多张抽卡结果选择局部优点，生成 composite target，再优化为统一 shader |

## 用户体验

### 精细控制面板

在 V2/V3 的 HumanLoopPanel 中新增 `Controls` 区域。由于 V2.1 Branch Canvas 已完成，V4 前端优先把该区域放入 `BranchCanvasInspector`，根据当前选中的 run/checkpoint/variant node 动态展示：
...
V4.5 增加 Local Fusion 工作流：

```text
Base: Card 2，整体最好
Water region: source Card 5，水面反射最好
Sky region: source Card 1，云层层次最好

Card 1 ───┐
Card 2 ───┼─ FusionPlan ── FusionResult
Card 5 ───┘
```

关键原则：

- base result 提供整体构图和 seed shader。
- source results 提供局部视觉目标。
- 系统先生成 `composite_target.png`，再从 base shader 创建 fusion child run。
- 最终交付仍是统一 shader，不是拼接图。
- V4 主文档只描述关系；详细 API、数据结构和测试见 [V4.5 local fusion supplement](2026-06-16-human-in-loop-v4-5-local-fusion-design.md)。
...
V4.5 不直接扩展 `branch-refine` 请求体作为主入口，而是新增 `/png-shader/fusions` 相关 endpoint。fusion run 最终仍会创建 child run，并在 lineage 中写入 `fusion_id`、`base_run_id`、`source_run_ids`。
...
- `PreferencePanel.tsx`
- `PreferenceChips.tsx`
- `RegionConstraintCanvasNode.tsx`，V4.2 基于 Branch Canvas 新增
- `PreferenceCanvasNode.tsx`，V4.3 基于 Branch Canvas 新增
- `FusionBuilderPanel.tsx`，V4.5 新增
- `FusionPlanCanvasNode.tsx`，V4.5 新增
...
基于已完成的 V2.1 Branch Canvas，V4 负责扩展以下职责：
...
- V4.5 的 `FusionPlanCanvasNode` 由 V4.5 自己新增，展示 base/source/output 关系，region 仍在 Main Preview 编辑。
...
- Branch Canvas 模式下，新增/删除 region 后，V4.2 扩展的 constraint node 与选中 run/checkpoint 同步。
- 切换 canvas node 时 inspector 中 constraints draft 不串到其他节点。
- V4.5 启用后，FusionBuilder 能创建 plan、生成 composite target，并启动 fusion child run。
...
| fusion source 缺少 render | V4.5 endpoint 返回 422；缺 selected GLSL 的 run 不能作为 base，但可作为视觉 source |
...
2. V4.1: 前端 FineControlPanel，不含 mask；基于已完成的 V2.1，先嵌入 BranchCanvasInspector。
3. V4.2: rectangle RegionMaskEditor + region constraints。
4. V4.2: 基于 Branch Canvas 新增 RegionConstraintCanvasNode，表达约束挂载关系。
...
8. V4.3: 基于 Branch Canvas 新增 PreferenceCanvasNode 或 preference annotation。
9. V4.4: preference notes 注入 branch/variant prompts。
10. V4.4: preference-assisted variant ranking。
11. V4.5: `fusion_plans.py` + `image_composite.py`，生成 composite target。
12. V4.5: fusion child run、fusion directed acceptance、Branch Canvas FusionPlanNode。
...
- `frontend/src/components/BranchCanvasInspector.tsx` — Branch Canvas 模式下 controls/preferences 的主要挂载点
- `frontend/src/lib/branchCanvasModel.ts` — V4 扩展 region/preference 附属节点映射
- [V4.5 local fusion supplement](2026-06-16-human-in-loop-v4-5-local-fusion-design.md) — 局部叠加、composite target、fusion child run

...
from p2s_agent.core.utils.glsl_postprocess import scan_undeclared_parameters
...
def validate_shader_static(source: str) -> dict:
    ...
    # 7. 未声明的全大写参数（postprocess 自动注入后兜底）
    undeclared = scan_undeclared_parameters(source)
    if undeclared:
        errors.append(
            "UNDECLARED: ALL_CAPS identifiers referenced without #define/const/uniform: "
            + ", ".join(undeclared[:10])
            + (f" (+{len(undeclared) - 10} more)" if len(undeclared) > 10 else "")
        )
